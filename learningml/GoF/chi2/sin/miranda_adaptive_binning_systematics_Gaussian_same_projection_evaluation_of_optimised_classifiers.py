...
if CPV:	
	#orig_name="chi2_gauss_rotated_perfectly_varysigma2_0_9__1_0_CPV_not_redefined_syst_"+str(systematics_fraction).replace(".","_")+"_"
	#orig_title="Gauss varysigma2 0.9 rot_perf syst{} adaptbin".format(systematics_fraction)
...
for dim_data in dim_list:
	...
	for i in range(100):
		if CPV: 
			#comp_file_list.append((os.environ['learningml']+"/GoF/data/gaussian_same_projection_on_each_axis/gauss_data/gaussian_same_projection_on_each_axis_rotated_perfectly_{1}D_10000_0.0_1.0_1.0_{0}.txt".format(i,dim_data),os.environ['learningml']+"/GoF/data/gaussian_same_projection_on_each_axis/gauss_data/gaussian_same_projection_on_each_axis_rotated_perfectly_{1}D_10000_0.0_varysigma2_0.9_1.0_{0}.txt".format(i,dim_data)))
			...
			#comp_file_list.append((os.environ['learningml']+"/GoF/data/gaussian_same_projection_on_each_axis/gauss_data/gaussian_same_projection_on_each_axis_rotated_perfectly_{1}D_10000_0.0_1.0_1.0_{0}.txt".format(i,dim_data),os.environ['learningml']+"/GoF/data/gaussian_same_projection_on_each_axis/gauss_data/gaussian_same_projection_on_each_axis_rotated_perfectly_{1}D_10000_0.0_1.0_1.0_1{0}.txt".format(str(i).zfill(2),dim_data)))
