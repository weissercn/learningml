	...
	#ml_classifiers          = ['nn']
        ...
        for ml_classifier_index, ml_classifier in enumerate(ml_classifiers):
                ...
                for dim in dimensions:
                        ...
                        ml_classifiers_dict[ml_classifier].append(np.loadtxt(os.environ['learningml']+"/GoF/optimisation_and_evaluation/"+ml_folder_name+"/"+ml_classifier+"/"+ml_file_name.format(dim,ml_classifier,ml_classifiers_bin))[1])


	ax.plot(dimensions, np.divide(ml_classifiers_dict['nn'],100.), label='ANN', color=ml_classifiers_colors[ml_classifier_index])
		...
		ax.plot(dimensions,np.divide(chi2_splits_dict[str(chi2_split)],100.), label=r"$\chi^2$", color= chi2_colors[chi2_split_index])	
...
if MODE == 'Gauss1_euclidean':
	...
	for ml_classifier_index, ml_classifier in enumerate(ml_classifiers):
		...
		ax.plot(dimensions, np.divide(ml_classifiers_dict[ml_classifier+str(ml_classifiers_bins[ml_classifier_index])],100.), label=ml_classifiers_labels[ml_classifier_index] , color=ml_classifiers_colors[ml_classifier_index])	
