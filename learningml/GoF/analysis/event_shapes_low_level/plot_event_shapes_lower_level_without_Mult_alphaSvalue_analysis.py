...
if MODE == 'lower_level':
	...
	#chi2_splits		= [8]
        ...
        ml_folder_name          = "automatisation_monash_low_level/evaluation_monash_low_level"
	chi2_without_Mult_folder_name        = "event_shapes_low_level"

	ml_file_name            = "{1}_monash_{0}_alphaSvalue_low_level_syst_0_01__chi2scoring_5_p_values"
	chi2_without_Mult_file_name          = "event_shapes_low_level_scale_uniform_syst_0_01__{0}D_chi2_{1}_splits_p_values"


	title                   = "Event shapes low level"
        name                    = "event_shapes_low_level"
	CL 			= 0.95  
	...
	if True:
		for ml_classifier_index, ml_classifier in enumerate(ml_classifiers):
			...
			ml_classifiers_dict[ml_classifier]= np.divide(ml_classifiers_dict[ml_classifier],100.)
                ...
                ax.errorbar(param_list,ml_classifiers_dict['bdt'], yerr=binomial_error(ml_classifiers_dict['bdt']), linestyle='-', marker='o', markeredgewidth=0.0, markersize=12, color=ml_classifiers_colors[0], label=r'$bdt low level$', clip_on=False)
		ax.errorbar(param_list,ml_classifiers_dict['nn'], yerr=binomial_error(ml_classifiers_dict['nn']), linestyle='-', marker='o', markeredgewidth=0.0, markersize=12, color=ml_classifiers_colors[2], label=r'$nn low level$', clip_on=False)
	...
	ax.errorbar(param_list,chi2_without_Mult_best, yerr=binomial_error(chi2_without_Mult_best), linestyle='--', marker='$\chi$', markeredgecolor='none', markersize=18, color='grey', label=r'$\chi^2 low level$',     clip_on=False)
	...
	#fig_name=name+"_alphaSvalue_analysis"
        fig_name="event_shapes_low_level_analysis"
